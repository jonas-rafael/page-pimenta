...
const whatsappLink = computed(() => `https://wa.me/${phone}?text=Olá!%20Gostaria%20de%20saber%20mais%20sobre%20a%20disponibilidade%20da%20Casa%20Pimenta.`)
    ...
    const { error } = await supabase
      ...
      .insert([
        { 
          ...
          telefone: bookingForm.value.telefone,
          check_in: bookingForm.value.dataInicio,
          check_out: bookingForm.value.dataFim,
          horario: bookingForm.value.horario,
          valor_total: pricing.value.total
        }
      ])
...
- *Valor Total:* R$ ${pricing.value.total},00
...
const amenities = [
  ...
  { name: 'Segurança 24h', icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' },
      ...
      <section id="galeria" class="mb-40">
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-6 auto-rows-[350px]">
          <div class="md:col-span-8 overflow-hidden rounded-3xl shadow-2xl group">
            <img src="/media/insta_casadepraiadopimenta_1(1).jpg" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000" />
          </div>
      ...
      <section id="agendar" class="mb-40">
        <div class="bg-white rounded-[4rem] shadow-2xl overflow-hidden border border-stone-100 flex flex-col lg:flex-row max-w-6xl mx-auto">
           <div class="lg:w-2/5 bg-stone-900 p-16 text-white flex flex-col justify-center space-y-8">
              ...
              <div class="space-y-2">
                <p class="text-emerald-400 font-bold text-xl">R$ 300 / diária</p>
           ...
           <form @submit.prevent="submitBooking" class="lg:w-3/5 p-16 bg-white space-y-10">
              ...
              <div v-if="pricing.error" class="p-6 bg-red-50 rounded-2xl border border-red-100 animate-in fade-in slide-in-from-bottom-2 duration-500">
                <p class="text-red-600 text-[10px] font-bold uppercase tracking-[0.3em] text-center">⚠️ {{ pricing.error }}</p>
              </div>

              <div v-if="pricing.total > 0" class="p-6 bg-stone-50 rounded-2xl border border-stone-100 flex justify-between items-center animate-in fade-in slide-in-from-bottom-2 duration-500">                <div>
                  <p class="text-[10px] font-bold text-stone-400 uppercase tracking-[0.3em] mb-1">Valor Total Estimado</p>
                  <p class="text-stone-500 text-xs font-medium">{{ pricing.days }} diárias</p>
                </div>
                <div class="text-right">
                  <p class="text-2xl font-black text-stone-900">R$ {{ pricing.total }},00</p>
                  <p v-if="pricing.isPackage" class="text-[9px] font-bold text-emerald-600 uppercase tracking-widest">Preço Especial Aplicado!</p>
                </div>
              </div>
              ...
              <p v-if="bookingStatus === 'error'" class="text-red-600 text-center text-[10px] font-bold uppercase tracking-[0.3em]">❌ Erro ao processar reserva. Tente novamente.</p>
           </form>
        </div>
      </section>
